...
class PostBloc extends Bloc<PostEvent, PostState> {
  ...
  void _searchItem(SearchItem event, Emitter<PostState> emit) {
    if (event.stSearch.isEmpty) {
      ...
      tempPostList = state.posts.where((element) => element.name.toString().toLowerCase().toString().contains(event.stSearch.toString().toLowerCase())).toList();
      // tempPostList = state.posts.where((map)=>map.userId.toString() == event.stSearch).toList();
      if (tempPostList.isEmpty) {
        emit(state.copyWith(tempSearchPostList: tempPostList, searchMessage: 'No found'));
      } else {
        emit(state.copyWith(tempSearchPostList: tempPostList, searchMessage: ''));
      }
    }
  }
}